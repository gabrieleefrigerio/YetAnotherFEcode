classdef AbaqusStructure < handle
    % ABAQUSSTRUCTURE Class to import an Abaqus mesh and create a YAFEC model.
    properties
        % --- Input file ---
        filename = ''; 
    ...
    methods
        ...
        function import_mesh(obj, filename)
            ...
            while ~feof(fid)
                ...
                elseif ~isempty(current_set_name)
                    ...
                    elseif strcmpi(current_set_name, 'ContactInterface')
                        obj.contact_nodes = [obj.contact_nodes; nums];
                    end
                end
            end
            fclose(fid);
            ...
            obj.contact_nodes = unique(obj.contact_nodes);
            
            fprintf('--- Mesh successfully imported from %s ---\n', filename);
            fprintf(' Total nodes: %d\n', size(obj.nodes, 1));
            fprintf(' Fixed Nodes (BottomFixed/TopFixed): %d\n', length(obj.bc_nodes));
            fprintf(' Contact Nodes (ContactInterface): %d\n', length(obj.contact_nodes));
        end
        ...
        function dofs_constrained = get_contact_dofs(obj, dir)
            % GET_CONTACT_DOFS Returns the DOFs for the contact interface.
            % Optional input: dir -> 1 for X direction, 2 for Y direction. Defaults to X.
            if nargin < 2
                dir = 1; 
            end
            
            if isempty(obj.contact_nodes)
                error('No contact nodes saved. Check that the .inp file contains "Nset, nset=ContactInterface".');
            end
            
            dofs_global = (obj.contact_nodes - 1) * obj.MeshObj.nDOFPerNode + dir;
